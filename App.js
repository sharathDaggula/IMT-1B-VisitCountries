...
class App extends Component {
  ...
  render() {
    ...
    const visitCountries = countriesList
    const visitedCountries = countriesList.filter(each => each.isVisited)
    return (
      <div className="bg-container">
        <h1>Countries</h1>
        <ul className="visit-countries-container">
          {visitCountries.map(each => (
            <li key={each.id}>
              <div className="visit-country-item-container">
                <p>{each.name}</p>
                {each.isVisited ? (
                  <p>visited</p>
                ) : (
          ...
          {visitedCountries.length > 0 ? (
            <ul className="bottom-container">
              {visitedCountries.map(each => (
                <li key={each.id}>
                  <div className="visited-country-item-container">
                    <img
                      src={each.imageUrl}
                      alt="thumbnail"
